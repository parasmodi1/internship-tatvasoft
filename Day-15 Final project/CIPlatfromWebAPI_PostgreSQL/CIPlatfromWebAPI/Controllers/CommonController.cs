...
namespace Web_API.Controllers
{
    ...
    public class CommonController : ControllerBase
    {
        ...
        public async Task<ResponseResult> AddUserSkill(UserSkill userSkill)
        {
            try
            {
                result.Data = await _balCommon.AddUserSkill(userSkill);
                result.Result = ResponseStatus.Success;
            }catch(Exception ex)
            {
                result.Message = ex.Message;
        ...
        public async Task<ResponseResult> GetUserSkill(int id)
        {
            try
            {
                result.Data = await _balCommon.GetUserSkill(id);
                result.Result = ResponseStatus.Success;
            }
            catch (Exception ex)
            {
